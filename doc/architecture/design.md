...
| 2026-07-03 · v1.1.0（Issue #3、PR #4） | Onboarding 死锁修复（方案 A 最小侵入）：① PermissionServiceProtocol 新增 `@MainActor requestScreenRecordingPrompt() -> Bool`，PermissionService 实现调 `CGRequestScreenCaptureAccess()` 触发 TCC 注册；Mock/Static 两 conformer 同步补齐，Mock 增 callLog 时序数组与 spy 基础设施。② OnboardingViewModel：`continueToNextStep` 进 `.screenRecording` 时、`openPermissionSettings(.screenRecording)` 打开设置前各触发一次 request；新增 `@MainActor skipOnboarding() async`（显式 `set(false, .clipboardEnabled)`、`set(true, .onboardingCompleted)`、hotkey 持久化、onComplete 回调）。③ OnboardingView：footer 左下角 Skip Setup 按钮 + SwiftUI `.alert` 二次确认，7 步全可见。④ Localizable.xcstrings 新增 5 键（en + zh-Hans）。⑤ AppDelegate.ensureScreenRecordingPermission NSAlert “打开设置”分支前补 request（对称 AC-7，代码层未注入，P2 遗留）。设计决策：`clipboardEnabled` 显式 false（PersistenceController 默认为 "true"，评审阻塞级发现，非 design 差异表原假设的 false）；按需申请复用 AppDelegate 现有 NSAlert 入口。遗留：AC-6/AC-7 端到端未验证（重启不重弹、hotkey 按需申请 Alert、SettingsViewModel 入口）。 |
